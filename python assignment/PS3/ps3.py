...
def get_word_score(word, n):
    ...
    for i in word.lower():
        first_component_score+=SCRABBLE_LETTER_VALUES.get(i)
    second_component_score=max(1,(7*len(word))-(3*(n-len(word))))
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in word:
        words_that_replace_wildcard = list_of_word
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand)>0:
      ...
      input_word=input("Please input  a word  or input !! to finish:")
      if input_word=="!!":
          break
      else:
          if is_valid_word(input_word,hand,word_list):
              current_hand_score=get_word_score(input_word,calculate_handlen(hand))
              total_score += current_hand_score
              print("The current  hand earned you",current_hand_score,"Your total score is",total_score)
          else:   
...
def substitute_hand(hand, letter):
    ...
    if not letter.isalpha() or letter not in tem_hand.keys():
        return tem_hand
    
    num_of_items_of_current_letter = tem_hand[letter]
    del(tem_hand[letter])
    ...
    while True:
        ...
        if new_letter not in hand.keys():
            tem_hand[new_letter] = num_of_items_of_current_letter
            break
