...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        possible_message=""
        for i in range(26):
            tem_text=self.apply_shift(i)
            words=tem_text.split()
            valid_word=sum([is_word(self.get_valid_word(),j)for j in words])
            if valid_word>most_valid_word:
                best_shift=i
                most_valid_word=valid_word
                possible_message=tem_text
        return (best_shift,possible_message)
